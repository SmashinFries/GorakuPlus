...
import { MD3DarkTheme, ProgressBar, Text, Tooltip, useTheme } from 'react-native-paper';
import { Media, MediaStatus, MediaType } from '../../../app/services/anilist/generated-anilist';
import { AnimatePresence, MotiView, useAnimationState } from 'moti';
import { MotiPressable } from 'moti/interactions';
import { StyleSheet, View, Image as RnImage } from 'react-native';
import { getScoreColor, listColor, rgbToRgba } from '../../../utils/colors';
import { LinearGradient } from 'expo-linear-gradient';
import { Image } from 'expo-image';
import { AiringBanner, AiringBannerMemo } from './episodeBanner';
import { ScoreHealthBar, ScoreIconText } from './itemScore';
...
type MediaItemTestProps = {
    item: Media;
    width?: number;
    height?: number;
    scoreGlow?: boolean;
    itemScore?: (avg: number, mean: number) => number;
    scoreColor?: (itemScore: number) => string;
    mediaLanguage?: 'romaji' | 'english' | 'native';
    navigate?: (aniID: number, malID: number, type: MediaType) => void;
};
export const MediaItemTest = ({
    item,
    scoreGlow,
    scoreColor,
    itemScore,
    ...
    const scoreColorMem = useMemo(
        () => scoreColor(itemScore(item.averageScore, item.meanScore)),
        [],
    );
    ...
    return (
        <MotiPressable
            ...
            containerStyle={{
                ...
                shadowRadius: scoreGlow ? (item.mediaListEntry?.status ? 20 : 10) : undefined,
                shadowOpacity: scoreGlow ? (item.mediaListEntry?.status ? 1 : 0.5) : undefined,
                backgroundColor: scoreGlow ? scoreColorMem : undefined,
            }}
            style={[
                MediaStyles.container,
                {
            ...
            <MotiView style={[MediaStyles.imgSize, MediaStyles.innerContainer]}>
                ...
                <Text
                    ...
                    style={{
                        ...
                        bottom:
                            (item?.nextAiringEpisode || item?.startDate) &&
                            !AIRING_BANNER_BANS.includes(item.status) &&
                            item?.type !== MediaType.Manga
                                ? '15%'
                                : 5,
                        fontWeight: 'bold',
                ...
                {item.type === MediaType.Anime && !AIRING_BANNER_BANS.includes(item.status) ? (
                    <AiringBanner
                        nextEpisode={item?.nextAiringEpisode}
                        format={item?.format}
                        startDate={item?.startDate}
...
type RenderMediaItemTestProps = {
    item: Media;
    index: number;
    delay?: boolean;
    navigate?: (aniID: number, malID: number, type: MediaType) => void;
};
export const RenderMediaItemTest = ({
    item,
    index,
    navigate,
    delay = true,
}: RenderMediaItemTestProps) => {
    const { colors } = useTheme();
    const { showItemListStatus } = useAppSelector((state) => state.persistedSettings);
    const MediaItemMemo = memo(MediaItem);
    const progress = useMemo(
