...
import { useMalQuery, useMangaUpdatesQuery, useMuReleasesQuery } from '@/api/anilist/extended';
...
export const useMedia = (aniId: number | null, type: MediaType) => {
	...
	const { mal: malDB, mangaUpdates: muDB } = useMatchStore();
	...
	const jikan = useMalQuery(malDB[aniId] ?? anilist.data?.Media?.idMal, type);
	...
	const muReleases = useMuReleasesQuery(muDB[aniId] ?? muSeries.data.data.series_id);

	return { anilist, jikan, muSeries, muReleases };
};
