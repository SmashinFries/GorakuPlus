...
export const useCalendar = () => {
    ...
    const getAllResults = async () => {
        ...
        const week_days:WeekData = {
            sunday: tempData.filter(
                (ep) =>
                    ep.airingAt > start && ep.airingAt < start + 86400,
            ),
            monday: tempData.filter(
                (ep) =>
                    ep.airingAt > start + 86400 &&
                    ep.airingAt < start + 86400 * 2,
            ),
            tuesday: tempData.filter(
                (ep) =>
                    ep.airingAt > start + 86400 * 2 &&
                    ep.airingAt < start + 86400 * 3,
            ),
            wednesday: tempData.filter(
                (ep) =>
                    ep.airingAt > start + 86400 * 3 &&
                    ep.airingAt < start + 86400 * 4,
            ),
            thursday: tempData.filter(
                (ep) =>
                    ep.airingAt > start + 86400 * 4 &&
                    ep.airingAt < start + 86400 * 5,
            ),
            friday: tempData.filter(
                (ep) =>
                    ep.airingAt > start + 86400 * 5 &&
                    ep.airingAt < start + 86400 * 6,
            ),
            saturday: tempData.filter(
                (ep) =>
                    ep.airingAt > start + 86400 * 6 && ep.airingAt < end,
            )
        }
        setData(week_days);
        setLoading(false);
