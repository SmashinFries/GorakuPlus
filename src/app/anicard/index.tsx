...
import { MediaAniCard } from '@/components/anicard/cards';
...
export type AniCardConfig = {
	titleLang?: keyof MediaTitle | undefined;
...
const MediaCardGenerator = ({
	...
	const actionsheetScrollRef = useRef<ScrollView>(null);
				...
				type && (
					<View style={{ flex: 1 }}>
						...
						>
							<MediaAniCard
								anilistScore={
									anilist?.data?.Media?.[config.scoreType ?? 'meanScore']
								}
								...
								description={
									config.enabledDescription
										? config.descriptionType === 'anilist'
											? (anilist?.data?.Media?.descriptionHTML ?? undefined)
											: config.descriptionType === 'mal'
												? (jikan?.data?.data?.data?.synopsis ?? undefined)
												: customDescription
										: undefined
								}
								isDescriptionHtml={config.descriptionType === 'anilist'}
								descriptionLines={config.descriptionNumLines}
								malScore={jikan?.data?.data?.data?.score ?? undefined}
								startDate={anilist?.data?.Media?.startDate ?? undefined}
								status={anilist?.data?.Media?.status ?? undefined}
								tags={[
									...(config.enableGenre
										? (anilist?.data?.Media?.genres ?? [])
										: [null]),
									...(anilist?.data?.Media?.tags?.map((tag) =>
										tag && config.enableTag ? tag.name : null,
									) ?? []),
								].filter((tag) => tag !== null)}
								...
								totalContent={
									type === MediaType.Anime
										? (anilist?.data?.Media?.episodes ?? undefined)
										: anilist?.data?.Media?.format === MediaFormat.Novel
											? (anilist?.data?.Media?.volumes ?? undefined)
											: (anilist?.data?.Media?.chapters ?? undefined)
								}
								title={
									anilist?.data?.Media?.title?.[config.titleLang ?? 'romaji'] ??
									''
								}
						...
						<BottomSheetParent
							scrollRef={actionsheetScrollRef}
							sheetRef={actionsheetRef}
							dismissible={false}
							dimmed={false}
							// sizes={[((height - cardHeight - 20) / height) * 100, '100%']}
							sizes={['44%', 'large']}
							...
							>
								...
								<View>
									<SegmentedButtons
										...
										value={config.titleLang ?? 'romaji'}
										onValueChange={(val) =>
											onConfigUpdate({ titleLang: val as keyof MediaTitle })
										}
									...
									<SegmentedButtons
										density="medium"
										value={config.titleSize ?? 'headlineSmall'}
										onValueChange={(val) =>
											onConfigUpdate({ titleSize: val as VariantProp<any> })
										}
										buttons={[
								...
								/> */}
									<View style={{ marginVertical: 12 }}>
										<Slider
											...
											description={`${config.titleNumLines ?? '∞'}`}
											initialValue={config.titleNumLines ?? 2}
											onValueUpdate={(val) =>
												onConfigUpdate({
								...
								<View>
									<SegmentedButtons
										...
										value={config.descriptionType ?? 'anilist'}
										onValueChange={(val) =>
											onConfigUpdate({
												descriptionType: val as
													| 'anilist'
													| 'mal'
													| 'custom'
													| undefined,
									...
									<Slider
										title="Number of Lines"
										description={`${config.descriptionNumLines}`}
										initialValue={config.descriptionNumLines ?? 'ANY'}
								...
								<View>
									...
									<Slider
										title="Limit"
										description={`${config.tagLimit}`}
										initialValue={config.tagLimit ?? 'ANY'}
										steps={1}
										maxValue={8}
										minValue={0}
										onValueUpdate={(val) =>
		...
		case 'media':
			return (
				<MediaCardGenerator
					id={parseInt(id)}
					type={mediaType}
					idMal={idMal ? parseInt(idMal) : undefined}
				/>
			);
		case 'character':
		case 'staff':
