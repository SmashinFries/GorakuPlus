	...
	endpoints: {
		...
		AnimeExplore: {
			...
			providesTags: (result) =>
				result
					? [
							...result.trending.media.map((media) => ({
								type: 'ExploreAnime' as const,
								id: media.id,
							})),
							...result.popular.media.map((media) => ({
								type: 'ExploreAnime' as const,
								id: media.id,
							})),
							...result.top.media.map((media) => ({
								type: 'ExploreAnime' as const,
								id: media.id,
							})),
							...result.thisSeason.media.map((media) => ({
								type: 'ExploreAnime' as const,
								id: media.id,
							})),
							...result.nextSeason.media.map((media) => ({
								type: 'ExploreAnime' as const,
								id: media.id,
							})),
							'ExploreAnime',
		...
		MangaExplore: {
			providesTags: (result) =>
				result
					? [
							...result.newReleases.media.map((media) => ({
